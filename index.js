...
let lon = 0;
const key = '';
//this api call gives 5 day forecast with 3-hour intervals
//let fiveDayApi = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${key}`;
//this api call gives current weather
...
// const fiveDayForecastContainer = document.getElementById('five-day');
// let isCurrent = true;
// let isFive = false;
// currentForecastContainer.style.display = 'block';
// fiveDayForecastContainer.style.display = 'none';

// const currentButton = document.querySelector('button#current-forecast-button');
...
// const fiveDayButton = document.querySelector('button#five-day-button');
// fiveDayButton.addEventListener('click', () => {
//   isFive = true;
//   isCurrent = false;
...
//array containing city objects used for latitude and longitude searching
const cities = [
    {
      "city": "New York",
...
//current forecast main weather window variables
const mainCityName = document.querySelector('.weather-container h1');
const mainCityTemp = document.querySelector('.weather-container h2');
const mainCityWeatherType = document.querySelector('.weather-container h3');
const mainCityWeatherIcon = document.querySelector('.weather-container img');
const mainCityTempFeelsLike = document.querySelector('.weather-container #feels-like');
const mainCityWind = document.querySelector('.weather-container #wind');
const mainSunrise = document.querySelector('.weather-container #sunrise');
const mainSunset = document.querySelector('.weather-container #sunset');

//five-day forecast weather window variables
// const fdCity = document.querySelector('#five-day h1');
// const fdForecast = [
//     {
//         date: document.querySelector('#day-1 h2'),
//         temp: document.querySelector('#day-1-temp'),
//         icon: '01d',
//         iconURL: `http://openweathermap.org/img/wn/01d@2x.png`,
//         img: document.querySelector('#day-1 img'),
//         type: document.querySelector('#day-1 h3'),
//     },
//     {
//         date: document.querySelector('#day-2 h2'),
//         temp: document.querySelector('#day-1-temp'),
//         icon: '01d',
//         iconURL: `http://openweathermap.org/img/wn/01d@2x.png`,
//         img: document.querySelector('#day-1 img'),
...
//     },
//     {
//         date: document.querySelector('#day-3 h2'),
//         temp: document.querySelector('#day-1-temp'),
//         icon: '01d',
//         iconURL: `http://openweathermap.org/img/wn/01d@2x.png`,
//         img: document.querySelector('#day-1 img'),
...
//     },
//     {
//         date: document.querySelector('#day-4 h2'),
//         temp: document.querySelector('#day-1-temp'),
//         icon: '01d',
//         iconURL: `http://openweathermap.org/img/wn/01d@2x.png`,
//         img: document.querySelector('#day-1 img'),
...
//     },
//     {
//         date: document.querySelector('#day-5 h2'),
//         temp: document.querySelector('#day-1-temp'),
//         icon: '01d',
//         iconURL: `http://openweathermap.org/img/wn/01d@2x.png`,
//         img: document.querySelector('#day-1 img'),
...
mainCityWeatherIcon.src = iconUrl;
mainCityWeatherType.textContent = 'Clear Skies';
...
searchBar.addEventListener('submit', e => {
    ...
    currentApi = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${key}`;
    //fiveDayApi = `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${key}`;
    ...
    // fetch(fiveDayApi)
    // .then(resp => resp.json())
    // .then(data => renderFdWeather(data));
...
function renderWeather(weatherData) {
    ...
    mainCityTemp.textContent = toFahrenheit(weatherData.main.temp) + '\u00b0';
    mainCityWeatherType.textContent = weatherData.weather[0].description;
    iconCode = weatherData.weather[0].icon;
    iconUrl = `http://openweathermap.org/img/wn/${iconCode}@2x.png`;
    mainCityWeatherIcon.src = iconUrl;
    mainCityTempFeelsLike.textContent = 'Feels like: ' + toFahrenheit(weatherData.main['feels_like']) + '\u00b0';
    mainCityWind.textContent = 'Wind: ' + weatherData.wind.speed + 'mph';
    mainSunrise.textContent = 'Sunrise: ' + convertTime(weatherData.sys.sunrise);
    mainSunset.textContent = 'Sunset: ' + convertTime(weatherData.sys.sunset);
}

// function renderFdWeather(weatherData) {
//     //renders 5-day weather
//     for (let i = 0; i < fdForecast.length; i++) {
//         for (let j = 4; j < weatherData.list.length; j += 8) {
//             console.log(fdForecast[i].date);
//             console.log(weatherData);
//             console.log(weatherData.list[j].dt_text);
//             fdForecast[i].date.textContent = weatherData.list[j].dt_text;
//             console.log(fdForecast[i].date);
//             fdForecast[i].temp.textContent = toFahrenheit(weatherData.list[j].main.temp) + '\u00b0';
//             fdForecast[i].icon = weatherData.list[j].weather[0].icon;
//             fdForecast[i].iconURL = `http://openweathermap.org/img/wn/${fdForecast[i].icon}@2x.png`;
//             fdForecast[i].img.src = fdForecast[i].iconURL;
//             fdForecast[i].type.textContent = weatherData.list[j].weather[0].description;
//         }
//     }
// }

function toFahrenheit (kelvin) {
    return Math.round((parseInt(kelvin, 10) - 273.15) * (9/5) + 32);
}

function convertTime(epoch) {
    //new Date takes milliseconds, multiply by 1000
...
function findCity(city) {
    let cityName = city;
    foundCity = cities.find(element => element.city === cityName);
  ...
  function displayNY() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=40.7127837&lon=-74.0059413&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("New York");
    
    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[0]
  }
  
  function displayLA() {
     // currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=34.0522342&lon=-118.2436849&appid=76887180fc0bdffa05b5bf61df624f5e`;
   
    currentApi = findCity("Los Angeles");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[1]
  }
  
  function displayChicago() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=40.7127837&lon=-74.0059413&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Chicago");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[2]
  }
  
  function displayHouston() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=29.7604267&lon=-95.3698028&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Houston");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[3]
  }
  
  function displayPhilly() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=39.9525839&lon=-75.1652215&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Philadelphia");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[4]
  }
  
  function displayPhoenix() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=33.4483771&lon=-112.0740373&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("Phoenix");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[5]
  }
  
  function displaySA() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=29.4241219&lon=-98.49362819999999&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("San Antonio");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[6]
  }
  
  function displaySD() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=32.715738&lon=-117.1610838&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("San Diego");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[7]
  }
  
  function displaySF() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=37.7749295&lon=-122.4194155&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("San Francisco");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[8]
  }
  
  function displayDetroit() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=42.331427&lon=-83.0457538&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Detroit");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[9]
  }
  
  function displayMemphis() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=35.1495343&lon=-90.0489801&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Memphis");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[10]
  }
  
  function displayDenver() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=39.7392358&lon=-104.990251&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("Denver");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[11]
  }
  
  function displayWashington() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=38.9071923&lon=-77.0368707&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("Washington");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[12]
  }
  
  function displayOC() {
    //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=35.4675602&lon=-97.5164276&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("Oklahoma City");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[13]
  }
  
  function displayPortland() {
      //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=45.5230622&lon=-122.6764816&appid=76887180fc0bdffa05b5bf61df624f5e`;
  
    currentApi = findCity("Portland");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[14]
  }
      
  function displayLV() {
     //currentApi = `https://api.openweathermap.org/data/2.5/weather?lat=36.1699412&lon=-115.1398296&appid=76887180fc0bdffa05b5bf61df624f5e`;
    
    currentApi = findCity("Las Vegas");

    fetch(currentApi)
    .then(resp => resp.json())
    .then(data => renderWeather(data));
    mainCityName.textContent = sidebarCities[15]
  }